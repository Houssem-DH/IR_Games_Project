...
from query_expansion import expand_query_based_on_country ,expand_query_based_on_synonyms
...
def handle_query(inverted_index, user_history):
    ...
    if query_text:
        ...
        tfidf_queries = calculate_tfidf_for_queries([(1, query_text)], inverted_index)
        tfidf_query = tfidf_queries[1]

        # Expand query based on manual synonyms
        expanded_query = expand_query_based_on_synonyms(tfidf_query.copy())

        # Use the expanded query in the retrieval process
        results = ranked_retrieval_tfidf([(1, expanded_query)], inverted_index)
...
def display_results(results, original_query, expanded_query):
    ...
    result_text.insert(tk.END, f"Expanded Query: {dict(expanded_query)}\n\n")
